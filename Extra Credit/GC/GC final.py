...
with open('rosalind_gc.txt', 'r') as in_file:
    ...
    dna = ''
    while True:
        line = in_file.readline()   # lines variable
        if not line:                # end if not line
            break
        line = line.strip()
        if "Rosalind" in line:      # Rosalind in line
            if len(dna) > 0:
                count = 0
                for char in dna:
                    if char=='G' or char=='C':
            ...
            idstr = line[1:]    # id string
            ids.append(idstr)   # id list
            dna = ''
        else:
            dna += line # combine  DNA into one string
 ##############################################################          
